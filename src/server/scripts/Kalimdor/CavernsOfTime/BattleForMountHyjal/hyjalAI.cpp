...
void hyjalAI::SpawnVeins()
{
    if (Faction == 0)
    {
        ...
        for (uint8 i = 0; i<7; ++i)
        {
            GameObject* gem = me->SummonGameObject(GO_ANCIENT_VEIN, Position(VeinPos[i][0], VeinPos[i][1], VeinPos[i][2], VeinPos[i][3]), G3D::Quat(0, 0, 0, 0), 0);
            if (gem)
            ...
            case JAINA:
                if (instance->GetData(DATA_ALLIANCE_RETREAT))
                {
                    ...
                    for (uint8 i = 0; i < 92; ++i)//summon fires
                        ...
                        me->SummonGameObject(GO_ROARING_FLAME, Position(AllianceFirePos[i][0], AllianceFirePos[i][1], AllianceFirePos[i][2], AllianceFirePos[i][3]), G3D::Quat(0, 0, 0, 0), 0);
                }
            ...
            case THRALL: //thrall
                if (instance->GetData(DATA_HORDE_RETREAT))
                {
                    ...
                    for (uint8 i = 0; i < 65; ++i)//summon fires
                        ...
                        me->SummonGameObject(GO_ROARING_FLAME, Position(HordeFirePos[i][0], HordeFirePos[i][1], HordeFirePos[i][2], HordeFirePos[i][3]), G3D::Quat(0, 0, 0, 0), 0);
                }
                else me->SetVisible(true);
                break;
        }
        ...
        if (!creatures.empty())
        {
            for (std::list<Creature*>::const_iterator itr = creatures.begin(); itr != creatures.end(); ++itr)
            {
                if ((*itr) && (*itr)->IsAlive() && (*itr) != me && (*itr)->GetEntry() != JAINA)
                {
                    ...
                    me->GetNearPoint(me, x, y, z, range, 0); // , me->GetAngle());
