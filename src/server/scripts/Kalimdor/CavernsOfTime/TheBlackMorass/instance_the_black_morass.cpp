...
public:
    ...
    struct instance_the_black_morass_InstanceMapScript : public InstanceScript
    {
        ...
        std::set<ObjectGuid> encounterNPCs;
        ...
        void CleanupInstance()
        {
            ...
            std::set<ObjectGuid> eCopy = encounterNPCs;
            for (std::set<ObjectGuid>::const_iterator itr = eCopy.begin(); itr != eCopy.end(); ++itr)
        ...
        void OnCreatureCreate(Creature* creature)
        {
            ...
            {
                case NPC_MEDIVH:
                    _medivhGUID.Set(creature->GetGUID());
                    break;
                case NPC_TIME_RIFT:
                    ...
                    if (!_shieldPercent)
                        if (Creature* medivh = instance->GetCreature(_medivhGUID))
                            if (medivh->IsAlive())
                            {
                                ...
                                std::set<ObjectGuid> eCopy = encounterNPCs;
                                for (std::set<ObjectGuid>::iterator itr = eCopy.begin(); itr != eCopy.end(); ++itr)
                                    if (Creature* creature = instance->GetCreature(*itr))
                                        creature->DespawnOrUnsummon();
                            }
                    break;
        ...
        void SetObjectData(uint32 type, ObjectGuid data)
        {
        ...
        ObjectGuid SetObjectData(uint32 data) const
        {
            if (data == DATA_MEDIVH)
                return _medivhGUID;
        ...
        {
            Creature* rift = NULL;
            for (std::set<ObjectGuid>::const_iterator itr = encounterNPCs.begin(); itr != encounterNPCs.end(); ++itr)
            ...
            Position pos;
            rift->GetNearPosition(0.0f, 2*M_PI*rand_norm());
