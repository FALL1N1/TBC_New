...
class npc_cork_gizelton : public CreatureScript
{
    public:
        ...
        struct npc_cork_gizeltonAI : public EscortAI
        {
            ...
            void SetGUID(ObjectGuid const& playerGUID, int32 faction = 0) override
            {
                _playerGUID.Set(playerGUID);
                _faction = faction;
            ...
            void SetData(uint32 field, uint32 data, Unit*)
            {
                if (field == 1 && data == 1)
...
public:
    ...
    struct npc_aged_dying_ancient_kodoAI : public ScriptedAI
    {
        ...
        void JustRespawned()
        {
            me->UpdateEntry(RAND(NPC_AGED_KODO, NPC_DYING_KODO, NPC_ANCIENT_KODO), NULL);
        }
        ...
        void SpellHit(Unit* caster, SpellInfo const* spell)
        {
            if (spell->Id == SPELL_KODO_KOMBO_ITEM)
            {
                ...
                {
                    me->UpdateEntry(NPC_TAMED_KODO, NULL);
                    EnterEvadeMode();
...
public:
    ...
    bool OnQuestAccept(Player* player, Creature* creature, Quest const* quest)
    {
       ...
       {
            if (EscortAI* escortAI = CAST_AI(npc_dalinda::npc_dalindaAI, creature->AI()))
            {
