            ...
            void SummonChannelers()
            {
                for (uint8 i = 0; i < 5; ++i)
                {
                    ...
                    if (!channeler || channeler->IsDead())
                        channeler = me->SummonCreature(ENTRY_CHANNELER, ShadowmoonChannelers[i][0], ShadowmoonChannelers[i][1], ShadowmoonChannelers[i][2], ShadowmoonChannelers[i][3], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 5*MINUTE*IN_MILLISECONDS);
                    if (channeler)
