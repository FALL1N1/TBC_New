        ...
        bool OnGossipHello(Player* player, GameObject* go)
        {
            if (InstanceScript* instance = go->GetInstanceScript())
                if (roll_chance_i(instance->GetBossState(DATA_THE_LURKER_BELOW) != DONE ? 25 : 0) && !instance->IsEncounterInProgress())
                {
                    ...
                    if (Creature* lurker = ObjectAccessor::GetCreature(*go, instance->GetObjectGuid(NPC_THE_LURKER_BELOW)))
...
class HasInLineCheck
{
    public:
        ...
        {
            return !_caster->HasInLine(unit, 5.0f, 5.0f) || (unit->GetTypeId() == TYPEID_UNIT && unit->ToUnit()->IsUnderWater());
        }
...
class spell_lurker_below_spout_cone : public SpellScriptLoader
{
    public:
        ...
        class spell_lurker_below_spout_cone_SpellScript : public SpellScript
        {
            ...
            void Register()
            {
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_lurker_below_spout_cone_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_CONE_ENEMY);
            }
        };
