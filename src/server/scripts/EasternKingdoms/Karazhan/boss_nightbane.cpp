...
public:
    ...
    struct boss_nightbaneAI : public ScriptedAI
    {
        ...
        void HandleTerraceDoors(bool open)
        {
            if (instance)
            {
                instance->HandleGameObject(instance->GetObjectGuid(DATA_MASTERS_TERRACE_DOOR_1), open);
                instance->HandleGameObject(instance->GetObjectGuid(DATA_MASTERS_TERRACE_DOOR_2), open);
            }
        }
    ...
    bool OnGossipHello(Player* pPlayer, GameObject *pGo)
    {
        if (InstanceScript* pInstance = pGo->GetInstanceScript())
        {
            if (pInstance->GetData(TYPE_NIGHTBANE) != DONE && !pGo->FindNearestCreature(17225, 40.0f))
                if (Creature *cr = ObjectAccessor::GetCreature(*pPlayer, pInstance->GetObjectGuid(DATA_NIGHTBANE)))
