...
public:
    ...
    struct npc_echo_of_medivhAI : public ScriptedAI
    {
        ...
        void RemoveCheats()
        {
            ...
            for (uint8 row = 0; row < 8; row++)
                for (uint8 col = 0; col < 8; col++)
                    if (ObjectGuid guid = board[row][col]->pieceGUID)
        ...
        void HandlePieceJustDied(Creature* piece)
        {
                ...
                switch (piece->GetEntry())
                {
                    ...
                    case NPC_KING_A:
                        if (instance->GetData(DATA_CHESS_EVENT) == IN_PROGRESS)
                        {
                            ...
                            me->SummonGameObject(DUST_COVERED_CHEST, -11058.0f, -1903.0f, 222.0f, 2.24f, G3D::Quat(0.f, 0.f, 0.f, 0.f), 7200000);
                            instance->SetData(DATA_CHESS_REINIT_PIECES, 0);
                        }
            ...
            else if (instance->GetData(CHESS_EVENT_TEAM) == ALLIANCE)
            {
                switch (piece->GetEntry())
                {
                    ...
                    case NPC_KING_H:
                        me->TextEmote("The halls of Karazhan shake, as the curse binding the doors of the Gamesman's Hall is lifted.");
        ...
        int HandlePieceRotate(Creature* piece, ObjectGuid trigger)
        {
            bool foundOld = false;
            bool foundNew = false;
            ...
            switch (urand(0, 2))
            {
                ...
                case 1: // Fire
                {
                    ...
                    if (targetList.size() > 3)
                        Trinity::Containers::RandomResize(targetList, 3);
                ...
                case 2: // Buff
                {
                    ...
                    Trinity::Containers::RandomResize(targetList, urand(1, 4));
        ...
        bool GossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId) override
        {
            ...
            uint32 chessPhase = instance->GetData(DATA_CHESS_GAME_PHASE);
            instance->SetData(CHESS_EVENT_TEAM, chessPhase < CHESS_PHASE_PVE_FINISHED ? player->GetTeam() : BOTH);
...
public:
    ...
    struct npc_chesspieceAI : public ScriptedAI
    {
        ...
        void MovementInform(uint32 MovementType, uint32 Data)
        {
            ...
            if (Creature* medivh = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_IMAGE_OF_MEDIVH)))
        ...
        void SpellHitTarget(Unit *target, const SpellInfo* spell)
        {
            ...
            {
                if (Creature* medivh = ObjectAccessor::GetCreature(*me, ObjectGuid(instance->GetGuidData(DATA_IMAGE_OF_MEDIVH))))
                {
        ...
        bool GossipHello(Player* player) override
        {
            ...
            if (player->GetTeam() == ALLIANCE && me->GetFaction() != A_FACTION && chessPhase < CHESS_PHASE_PVE_FINISHED)
                return true;

            if (player->GetTeam() == HORDE && me->GetFaction() != H_FACTION && chessPhase < CHESS_PHASE_PVE_FINISHED)
                return true;
        ...
        bool GossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId) override
        {
            ...
            if (action == GOSSIP_ACTION_INFO_DEF + 1)
            {
                ...
                switch (me->GetEntry())
                {
                    ...
                    default:
                        TC_LOG_INFO("server.worldserver", "Karazhan Chess, Default hit at %u", me->GetEntry());
                        break;
                }
            }
            CloseGossipMenuFor(player);
