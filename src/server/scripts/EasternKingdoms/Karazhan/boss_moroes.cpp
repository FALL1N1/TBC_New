        ...
        void SpawnAdds()
        {
            ...
            if (isAddlistEmpty())
            {
                ...
                for (std::list<uint32>::const_iterator itr = AddList.begin(); itr != AddList.end() && i < 4; ++itr, ++i)
                {
                    ...
                    if (Creature* creature = me->SummonCreature(entry, Locations[i], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10 * SECOND * IN_MILLISECONDS))
                    {
            ...
            else
            {
                for (uint8 i = 0; i < 4; ++i)
                {
                    if (Creature* creature = me->SummonCreature(AddId[i], Locations[i], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10 * SECOND * IN_MILLISECONDS))
                        AddGUID[i] = creature->GetGUID();
                }
            }
        }
