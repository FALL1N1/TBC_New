...
public:
    ...
    bool OnQuestAccept(Player* player, Creature* creature, const Quest *quest)
    {
        if (quest->GetQuestId() == QUEST_PYREWOOD_AMBUSH && !CAST_AI(pyrewood_ambush::pyrewood_ambushAI, creature->AI())->QuestInProgress)
        {
            ...
            CAST_AI(pyrewood_ambush::pyrewood_ambushAI, creature->AI())->PlayerGUID.Set(player->GetGUID());
        }
            ...
            switch (Phase)
            {
                case 0:
                    if (WaitTimer == WAIT_SECS)
                        me->Say(NPCSAY_INIT, LANG_UNIVERSAL, NULL); //no blizzlike
                ...
                case 5: //end
                    if (PlayerGUID)
                    {
                        if (Player* player = ObjectAccessor::GetPlayer(*me, PlayerGUID))
                        {
                            me->Say(NPCSAY_END, LANG_UNIVERSAL, NULL); //not blizzlike
