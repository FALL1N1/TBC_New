...
public:
    ...
    {
        npc_demonic_vaporAI(Creature* creature) : ScriptedAI(creature) { }
        ...
        void UpdateAI(uint32 diff)
        { 
            if (me->GetMotionMaster()->GetCurrentMovementGeneratorType(MOTION_SLOT_DEFAULT) == IDLE_MOTION_TYPE)
            //if (me->GetMotionMaster().GetMotionSlotType(MOTION_SLOT_CONTROLLED) == NULL_MOTION_TYPE)
            {
                ...
                for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                    if (me->GetDistance2d(itr->GetSource()) < 20.0f && itr->GetSource()->IsAlive())
                    {
                        ...
                        me->GetMotionMaster()->MoveFollow(itr->GetSource(), 0.0f, 0.0f, MOTION_SLOT_ACTIVE);
                        break;
                    }
            }
        }
...
public:
    ...
    struct npc_demonic_vapor_trailAI : public ScriptedAI
    {
        npc_demonic_vapor_trailAI(Creature* creature) : ScriptedAI(creature)
        {
            timer = 1;
        }
...
class DoorsGuidCheck
{
    public:
        bool operator()(WorldObject* object) const
        {
            ...
            // GetDBTableGUIDLow
            return cr->GetGUID() != 54780 && cr->GetGUID() != 54787 && cr->GetGUID() != 54801;
        }
};
